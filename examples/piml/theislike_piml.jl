...
for epoch in epochs
    ...
    push!(train_time, tt)
    loss_train = sum(pmap(x->loss(x,train=false), 1:64, data_train_batch))
    loss_test = sum(pmap(x->loss(x,train=false), 1:64, data_test))
    rmse_train = sqrt(loss_train/length(data_train_batch))
    rmse_test = sqrt(loss_test/length(data_test))
    diffs_train = pmap(x->mydiff(x,train=false), 1:64, data_train_batch)
    diffs_test = pmap(x->mydiff(x,train=false), 1:64, data_test)
